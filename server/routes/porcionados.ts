...
porcionadosRouter.get('/', async (req, res) => {
  try {
    ...
    const deliveries = await prisma.delivery.findMany({
      ...
      include: { hamburguesas: true, client: true },
    });

    const aggregated = new Map<string, { producto: string; gramaje: number; cantidad: number; nombre_cliente: string }>();
    ...
    deliveries.forEach((delivery) => {
      delivery.hamburguesas.forEach((h) => {
        ...
        const nombre_cliente = delivery.client?.nombre || '';
        const key = `${producto}-${gramaje}`;
        const current = aggregated.get(key);
        if (current) {
          current.cantidad += cantidad;
        } else {
          aggregated.set(key, { producto, gramaje, cantidad, nombre_cliente });
        }
      });
    });
    ...
    const existingPorcionados = await prisma.porcionado.findMany({
      ...
      select: { id: true, producto: true, gramaje: true, estado: true, nombre_cliente: true },
    });
    ...
    const merged = items.map((it) => {
      ...
      return {
        ...
        nombre_cliente: match ? match.nombre_cliente || it.nombre_cliente : it.nombre_cliente,
      };
    });
...
porcionadosRouter.post('/mark', async (req, res) => {
  try {
    const { producto, gramaje, cantidad, fecha, nombre_cliente } = req.body || {};
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!target) {
        ...
        await tx.porcionado.update({ where: { id: target.id }, data: { cantidad: Number(cantidad), nombre_cliente: nombre_cliente || target.nombre_cliente } });
